export default {
  ...
  getErrorText(error) {
    const parts = [error?.message, error?.error, error?.data, error?.responseMeta?.body];
  ...
  isTokenExpiredError(error) {
    ...
    return text.includes("token expired") || text.includes("token_expired") || text.includes("jwt expired");
  },

  async refreshAccessToken() {
    if (items.refreshPromise) return await items.refreshPromise;
  ...
  createItems: async (params = {}) => {
    ...
    return await items.runWithRefresh(() => qCreateItems.run({ fields, filter, body, limit, collection }));
  },

  updateItems: async (params = {}) => {
    const { fields = "*", collection, filter = {}, body = {}, limit = -1 } = params;
    if (!body || !collection) throw new Error("Both 'body' and 'collection' must be defined.");
    return await items.runWithRefresh(() => qUpdateItems.run({ fields, filter, body, limit, collection }));
  },

  getItems: async (params = {}) => {
    const { fields = "*", collection, filter = {}, body = {}, limit = -1 } = params;
    if (!fields || !collection) throw new Error("Both 'fields' and 'collection' must be defined.");
    return await items.runWithRefresh(() => qGetItems.run({ fields, filter, body, limit, collection }));
  },

  deleteItems: async (params = {}) => {
    const { fields = "*", collection, filter = {}, body = {}, limit = -1 } = params;
    if (!body || Object.keys(body).length === 0) throw new Error("You must specify body for deletion!");
    return await items.runWithRefresh(() => qDeleteItems.run({ fields, filter, body, limit, collection }));
  },
  ...
  async ensureFreshToken(bufferMs = 60000) {
    ...
    if (!items.isTokenExpiringSoon(bufferMs)) return;
    await items.refreshAccessToken();
  }
}